...
ISBN nvarchar(50) constraint FK_kitaplar_bulunur foreign key(ISBN) references kitaplar(ISBN),
);
create table yazar(
yazarNumber int not null primary key identity(1,1),
yazarNo int constraint FK_yazarlar_yazar foreign key (yazarNo) references yazarlar(yazarNo),
ISBN nvarchar(50) constraint FK_kitaplar_yazar foreign key (ISBN) references kitaplar(ISBN),
);
create table bul(
bulNo int not null primary key identity(1,1),
kategoriNo int constraint FK_kategoriler_bul foreign key(kategoriNo) references kategoriler(kategoriNo),
ISBN nvarchar(50) constraint FK_kitaplar_bul foreign key (ISBN) references kitaplar(ISBN),
);
